...
func TestPIDEqualityUsesAllIdentityDimensions(t *testing.T) {
	...
	if !PidEqual(base, base) { t.Fatal("same normalized PID must compare equal") }
	...
	} { t.Run(name, func(t *testing.T) { if PidEqual(base, changed) { t.Fatalf("PID differing in %s must not compare equal", name) } }) }
}
...
func TestRuntimeNeutralHelpersAcceptOpaqueMessages(t *testing.T) {
	...
	if err := Send(context.Background(), pid, message); err != nil { t.Fatal(err) }
	if _, err := Call(context.Background(), pid, message, time.Second); err != nil { t.Fatal(err) }
	if runtime.sent != message { t.Fatalf("sent %#v, want %#v", runtime.sent, message) }
}

type testRuntime struct { sent any; localSent any; deregister string }
...
func (r *testRuntime) ActivateActor(context.Context, string, string, bool) (PID, error) { return PID{}, nil }
...
func (r *testRuntime) LocalSend(_ context.Context, _ PID, message any) error { r.localSent = message; return nil }
func (r *testRuntime) Call(_ context.Context, _ PID, message any, _ time.Duration) (any, error) { r.sent = message; return struct{}{}, nil }
...
func TestBusinessActorContractHasNoRuntimeActorRequirement(t *testing.T) { var _ ActorProducer = func() IActor { return &contractActor{} } }
...
func TestLegacyPIDRoundTripPreservesTransportAddress(t *testing.T) {
	...
	if pid.Node != app.nodeInstanceName { t.Fatalf("normalized node = %q, want %q", pid.Node, app.nodeInstanceName) }
	roundTrip := protoFromPID(pid)
...
func TestActorBaseStoppedPreservesStopReason(t *testing.T) {
	...
	base.stopErr = reason
	if err := base.doReceive(&testActorContext{message: ActorStoppedMessage{}}); err != nil { t.Fatal(err) }
	if actorImpl.terminatedErr != reason { t.Fatalf("Terminate error = %v, want %v", actorImpl.terminatedErr, reason) }
}
...
func TestRuntimeDispatchesLocalSendAndDeregister(t *testing.T) {
	...
	if err := LocalSend(context.Background(), pid, message); err != nil { t.Fatal(err) }
	DeregisterActorKind("role")
	if runtime.localSent != message || runtime.deregister != "role" { t.Fatalf("local=%#v deregister=%q", runtime.localSent, runtime.deregister) }
}

func TestUninitializedHelpersReturnErrors(t *testing.T) {
	oldApp := app; app = nil; SetRuntime(nil); t.Cleanup(func() { app = oldApp })
	pid := PID{Runtime: "test", Node: "node", ID: "id", Creation: "1"}
	if err := Send(context.Background(), pid, struct{}{}); err == nil { t.Fatal("Send unexpectedly succeeded") }
	if _, err := Call(context.Background(), pid, struct{}{}, time.Second); err == nil { t.Fatal("Call unexpectedly succeeded") }
	if _, err := ActivateActor(context.Background(), "role", "1", true); err == nil { t.Fatal("ActivateActor unexpectedly succeeded") }
	if err := Stop(pid); err == nil { t.Fatal("Stop unexpectedly succeeded") }
}
