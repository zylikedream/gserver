...
// leastWeightRandomSelector 实现选择权重最小节点的选择器
// 当有多个相同权重的最小节点时，随机选择其中一个

type leastWeightRandomSelector struct {
}

var leastWeightRandomSelectorInstance = &leastWeightRandomSelector{}

// LeastWeightRandomSelector 返回最小权重随机选择器实例
func LeastWeightRandomSelector() ServiceSelector {
	return leastWeightRandomSelectorInstance
}

// Select 选择权重最小的节点，如果有多个相同权重的最小节点，则随机选择一个
// 算法原理：
// 1. 找出所有权重最小的节点
// 2. 如果只有一个最小权重节点，直接返回
// 3. 如果有多个最小权重节点，随机选择其中一个
func (s *leastWeightRandomSelector) Select(service string, nodes []ServiceNode) ServiceNode {
	if len(nodes) == 0 {
		return ServiceNode{}
	}

	// 找出最小权重
	minWeight := int(^uint(0) >> 1) // 初始化为最大整数值
	for _, node := range nodes {
		currentWeight := node.Weight
		if currentWeight <= 0 {
			currentWeight = 1 // 权重为0或负数时，默认视为1
		}
	...
	// 收集所有权重最小的节点
	minWeightNodes := make([]ServiceNode, 0)
	for _, node := range nodes {
		currentWeight := node.Weight
		if currentWeight <= 0 {
			currentWeight = 1 // 权重为0或负数时，默认视为1
		...
		if currentWeight == minWeight {
			minWeightNodes = append(minWeightNodes, node)
		}
	}

	// 从最小权重节点列表中随机选择一个
	return minWeightNodes[rand.Intn(len(minWeightNodes))]
}
