...
func TestRoleMainInitRequiresAccountRecord(t *testing.T) {
	...
	r := NewRoleMain()
	err := r.Init(newRoleTestContext(), []any{int64(1001), gxyactor.ActorOwner{NodeID: "role-1@instance-a", Epoch: 1}})
	if err == nil {
...
func TestRoleMainInitAcceptsExistingAccountRecord(t *testing.T) {
	...
	r := NewRoleMain()
	if err := r.Init(newRoleTestContext(), []any{int64(1001), gxyactor.ActorOwner{NodeID: "role-1@instance-a", Epoch: 1}}); err != nil {
...
func TestRoleMainInitRequiresActorOwner(t *testing.T) {
	r := NewRoleMain()
	err := r.Init(newRoleTestContext(), []any{int64(1001)})
