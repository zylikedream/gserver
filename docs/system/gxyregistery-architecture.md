...
**存储**: Redis Hash `gserver:svc:{serviceName}`，field=服务 key（`gserver-{NodeName}-{Name}:{NodeHost}`，即 `ServiceInfo.GetKey()`），value=JSON

```json
{"Name":"role","NodeName":"role-0@18af...","Version":"v1.0.0","Weight":0,"NodeHost":"10.244.0.152:19001"}
```

Hash field 示例：`gserver-role-0@18af...:10.244.0.152:19001`
