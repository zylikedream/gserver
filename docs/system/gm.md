...
GM（Game Master）命令系统，通过游戏协议提供开发测试命令。命令定义在 `RoleGM` 模块中，带 `用法:` 注释的方法通过 `go/doc` 自动提取为帮助文档。
...
新增命令只需在 `role_gm.go` 中添加导出方法并写好包含 `用法:` 的注释即可，无需改动其他文件。
...
客户端发 ReqGMCommand {cmd: "add_goods 1001 10"}
  ...
  → RoleGM.execCommand("add_goods", ["1001", "10"])
  → 反射查找 add_goods → RoleGM.AddGoods(1001, 10)
  → r.Role.Bag.AddItem(...)
...
| `src/apps/role/internal/logic/role_gm.go` | RoleGM 模块（命令定义 + 帮助提取 + 反射路由） |
...
| 命令定义 | RoleGM 方法 | 带 `用法:` 注释的方法被 go/doc 识别为命令 |
