...
```
| Length | Type | PathLen | Path | Payload |
```

- Length、Type、PathLen 的字节数及大小端由 `gxynet.packet.ltpv` 配置决定
- Length: Type + PathLen + Path + Payload 的长度，不包含 Length 字段自身
- Type: 消息类型
- PathLen: 路径字符串长度
- Path: 消息名称路径（用于消息路由）
...
客户端 TCP 数据
  → gnet 事件循环
  → TcpServer.OnTraffic 从 gnet.Conn 读取并循环 LTPV 解码
  → TcpEndpoint + Processor 路由
