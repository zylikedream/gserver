...
### 1. Actor 定位依赖 Redis TTL
- **位置**: `core/gxyactor/activator_manager.go`
- **问题**: Actor 在 Redis 中的注册使用 40s TTL + 30s 刷新，余量仅 10s。如果 Redis 不可用超过 40s，Actor 位置会丢失，导致重复创建
- **风险**: 中等 — Redis 通常高可用，但 TTL 机制存在窗口期
- **建议**: 考虑增大 TTL/刷新间隔比例（如 TTL=90s, 刷新=30s），或本地缓存作为 fallback，增加续约重试机制

### 2. Supervisor 策略过于简单
- **位置**: `core/gxyactor/system.go:167-169`
...
### 3. 无消息重试机制
- **位置**: `src/apps/gateway/internal/logic/session.go:201`
- **问题**: Session 向 RoleMain 发送消息使用 `CallSync`（fire-and-forget），如果 RoleMain 暂时不可用（正在迁移、重启），消息会丢失
- **风险**: 中等 — 客户端请求可能丢失，需要客户端重试或服务端消息队列

### 4. 批量续约不校验所有权
- **位置**: `core/gxylocator/gxylocator.go:76-89`
...
### 7. 保存失败直接停止 Actor
- **位置**: `src/apps/role/internal/logic/role_main.go:284-290`
- **问题**: 定时保存失败时直接调用 `r.Stop(err)` 终止 Actor。如果是临时性数据库故障，会导致大量角色被踢下线
- **风险**: 中等 — 应考虑重试机制或降级策略

### 8. GORM Save 不做冲突检测
- **位置**: `src/apps/role/internal/logic/role_main.go:309`
- **问题**: 使用 `db.Save(modState)` 保存，GORM 的 Save 会更新所有字段。如果同一 Actor 被意外在两个节点激活（Redis 定位失败场景），可能导致数据覆盖
- **现状**: 依赖 Actor 单例保证（Redis SETNX + TTL），正常流程不会出现
- **风险**: 低 — Actor 模型保证串行，但缺少兜底保护
...
### 10. actorActivator.childs 并发访问
- **位置**: `core/gxyactor/activator_manager.go`
- **问题**: `childs map[PID]string` 在 actorActivator 中被读写。由于 Actor 串行处理消息，这实际上是安全的
- **风险**: 无 — Actor 模型保证串行访问

## 可观测性

### 11. 缺乏监控指标
- **问题**: 没有 Prometheus/metrics 暴露，无法监控在线人数、消息延迟、Actor 分布等关键指标
...
### 13. Logger WithAttrs/WithGroup 丢失字段
- **位置**: `core/gxyactor/logger.go`
- **问题**: `actorLogAdapter` 的 `WithAttrs` 和 `WithGroup` 返回 `self` 而非新实例，protoactor-go 通过 `With()` 附加的系统字段被静默丢弃
- **风险**: 低 — 影响 Actor 系统内部日志的结构化字段

## 代码质量

### 14. protojson.Marshal 错误被忽略
- **位置**: `core/gxyactor/activator_manager.go:170,179,319`
- **问题**: 三处 `protojson.Marshal` 的错误返回用 `_` 丢弃。如果序列化失败，空字符串会被注册为 Actor PID，导致 Actor 不可定位
