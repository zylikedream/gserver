...
- 日期：2026-09-03
- 修订：2026-09-04（2026-09-18 标注部分取代）
- 状态：**Accepted**
- 关联：Actor Locate 并发压测与所有权改造
- **部分取代**：决策 3 中「`nodeInstanceName` 同时作为唯一进程身份和 lease token」已由 **ADR 0010** 取代（路由身份与所有权身份分离，令牌改为每实例随机）。本节下方决策 3 的原文保留作为历史，实现以 ADR 0010 为准；本 ADR 的其余决策全部有效。
