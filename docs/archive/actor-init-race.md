...
> **已归档**（2026-09-19）。本文描述的是旧运行时的缺陷：其"创建与初始化分离"使
> 调用方可能在初始化失败后仍拿到进程标识。新运行时的创建是**同步**的——返回时初始化
> 已完成或已失败，失败即进程终止，不存在该窗口。问题前提消失，故归档保留历史。
> 现行设计见 [ADR 0012](../architecture/adr-0012-actor-activation-and-ownership.md)。
